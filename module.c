...
//TODO: Apply consistent code style
...
static PyObject *_parse(PyObject *self, PyObject *args, int parse_any_tzinfo)
{
    PyObject *obj;
    ...
    // Python 3.6+ does this validation as part of Datetime's C API constructor. See https://github.com/python/cpython/commit/b67f0967386a9c9041166d2bbe0a421bd81e10bc
    // If ciso8601 were to drop Python <3.6 support, it might still be useful, since it is short-circuit parsing.
    // We skip ` || year < datetime.MAXYEAR)`, since cio8601 currently doesn't support 5 character years, so it is impossible. 
    ...
    if (*c == '-'){ //Separated Month and Day (ie. MM-DD)
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            {
                return format_unexpected_character_exception("month", *c, (c - str) / sizeof(char), 2 - i);
            }
        }
        if (*c != '\0' && !IS_DATE_AND_TIME_SEPARATOR){ //Optional Day
            if (*c == '-'){
                ...
                PyErr_Format(PyExc_ValueError, "Invalid character while parsing date separator ('-') ('%c', Index: %d)", *c, (c - str) / sizeof(char));
                return NULL;
            }
            //Day
            for (i = 0; i < 2; i++)
            {
                ...
                {
                    return format_unexpected_character_exception("day", *c, (c - str) / sizeof(char), 2 - i);
                }
            }
        } else {
            day = 1;
        ...
        // Note that YYMM is not a valid timestamp. If the calendar date is not separated, a day is required (ie. YYMMDD) 
        for (i = 0; i < 2; i++)
        {
            if (*c >= '0' && *c <= '9')
            {
    ...
    // These checks are needed for Python <3.5 support. See https://github.com/closeio/ciso8601/pull/30
    // Python 3.6+ does this validation as part of Datetime's C API constructor. See https://github.com/python/cpython/commit/b67f0967386a9c9041166d2bbe0a421bd81e10bc
    // If ciso8601 were to drop Python <3.5 support, these might still be useful, since they short-circuit parsing.
    if (month < 1 || month > 12)
    {
    ...
    // Only needed for Python <3.5 support (or performance short-circuiting. Python 3.6+ does this validation as part of Datetime's constructor).
    if (day < 1)
    {
    ...
    if (*c != '\0'){
        ...
        if (IS_DATE_AND_TIME_SEPARATOR){
            ...
            PyErr_Format(PyExc_ValueError, "Invalid character while parsing date and time separator (ie. 'T' or ' ') ('%c', Index: %d)", *c, (c - str) / sizeof(char));
            return NULL;
        }
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            {
                return format_unexpected_character_exception("hour", *c, (c - str) / sizeof(char), 2 - i);
            }
        }
        if (*c != '\0' && !IS_TIME_ZONE_SEPARATOR){ //Optional minute and second
            ...
            if (*c == ':'){ //Separated Minute and Second (ie. mm:ss)
                ...
                for (i = 0; i < 2; i++)
                {
                    ...
                    {
                        return format_unexpected_character_exception("minute", *c, (c - str) / sizeof(char), 2 - i);
                    }
                }
                if (*c != '\0' && !IS_TIME_ZONE_SEPARATOR){ // Optional Second
                    if (*c == ':'){
                        c++;
                    } else {
                        PyErr_Format(PyExc_ValueError, "Invalid character while parsing time separator (':') ('%c', Index: %d)", *c, (c - str) / sizeof(char));
                        return NULL;
                    }
                    //Second
                    for (i = 0; i < 2; i++)
                    {
                        ...
                        {
                            return format_unexpected_character_exception("second", *c, (c - str) / sizeof(char), 2 - i);
                        }
                    }
                    ...
                    if (IS_FRACTIONAL_SEPARATOR){
                        ...
                        for (i = 0; i < 6; i++)
                        {
                            ...
                            {
                                //We need at least one digit. Trailing '.' or ',' is not allowed
        ...
        if (IS_TIME_ZONE_SEPARATOR){
            ...
            if (tzsign != 0)
            {
                ...
                for (i = 0; i < 2; i++)
                {
                    ...
                    {
                        return format_unexpected_character_exception("tz hour", *c, (c - str) / sizeof(char), 2 - i);
                    }
                }
                ...
                { // Optional separator
                    ...
                    for (i = 0; i < 2; i++)
                    {
                        ...
                        {
                            return format_unexpected_character_exception("tz minute", *c, (c - str) / sizeof(char), 2 - i);
                        }
                    }
                }
                else if (*c != '\0')
            ...
            // It's not entirely clear whether this validation check is necessary under ISO 8601.
            // For now, we will error on the side of caution and prevent suspected invalid timestamps
            // If we need to loosen this restriction later, we can.
            if (tzminute > 59)
            {
            ...
            if (aware && parse_any_tzinfo)
            {
                if (pytz_fixed_offset == NULL || pytz_utc == NULL){
                    PyErr_SetString(PyExc_ImportError, "Cannot parse an aware timestamp without pytz. Install it with `pip install pytz`.");
                    return NULL;
                }
...
static PyObject* parse_datetime_as_naive(PyObject* self, PyObject* args)
{
    return _parse(self, args, 0);
}

PyObject* parse_datetime(PyObject* self, PyObject* args)
{
...
static PyMethodDef CISO8601Methods[] =
    {
        ...
        {"parse_datetime_as_naive", parse_datetime_as_naive, METH_VARARGS, "Parse a ISO8601 date time string, ignoring the time zone component."},
        {NULL, NULL, 0, NULL}};
