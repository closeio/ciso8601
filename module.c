...
static PyObject *_parse(PyObject *self, PyObject *args, int parse_any_tzinfo) {
  ...
  if (*c != '\0') {
      ...
      if (*c == ':') {  // Separated Minute and Second (ie. mm:ss)
        ...
        if (*c != '\0' && !IS_TIME_ZONE_SEPARATOR) {  // Optional Second
          if (*c == ':') {
            ...
            PyErr_Format(PyExc_ValueError,
                         "Invalid character while parsing time separator (':') "
                         "('%c', Index: %d)",
                         *c, (c - str) / sizeof(char));
            return NULL;
          }
          ...
          if (IS_FRACTIONAL_SEPARATOR) {
            ...
            for (i = 0; i < 6; i++) {
              if (*c >= '0' && *c <= '9') {
                ...
                // We need at least one digit. Trailing '.' or ',' is not
                // allowed
                return format_unexpected_character_exception(
    ...
    if (hour == 24 && minute == 0 && second == 0 && usecond == 0) {
      // Special case of 24:00:00, that is allowed in ISO 8601. It is equivalent
      // to 00:00:00 the following day
    ...
    if (IS_TIME_ZONE_SEPARATOR) {
        ...
        if (*c == ':') {  // Optional separator
          ...
          for (i = 0; i < 2; i++) {
            if (*c >= '0' && *c <= '9') {
              ...
              return format_unexpected_character_exception(
                  "tz minute", *c, (c - str) / sizeof(char), 2 - i);
            }
          }
      ...
      // It's not entirely clear whether this validation check is necessary
      // under ISO 8601. For now, we will error on the side of caution and
      // prevent suspected invalid timestamps If we need to loosen this
      // restriction later, we can.
      if (tzminute > 59) {
  ...
  obj = PyDateTimeAPI->DateTime_FromDateAndTime(year, month, day, hour, minute,
